...
// changed to accept peak coordinates from tensorflow::ops::Where instead of peak maps from. tf.Where
#include "pch.h"
...
// TODO define coords and review, wire up in estimate_paf, refactor pafprocess into C++ class, check result to contain a single human
#define COORDS(i, j) coords[j+4*i]

void gather_peak_infos(const int c1, const INT64 * coords, const int p1, const int p2, const int p3, const float * peaks, const int h2, const int h3, const float * heatmap, std::vector<Peak> peak_infos[NUM_PART]) {
	int peak_cnt = 0;
	for (int c = 0; c < c1; c++) {
		// TODO Wrong index
		int x = COORDS(c, 2);
		int y = COORDS(c, 1);
		// TODO Resolve NUM_PART == 18
		int part_id = COORDS(c, 3);		{ part_id = min(part_id, NUM_PART -1);
...
int process_paf(const int p1, const int p2, const int p3, const float *peaks, const int h1, const int h2, const int h3, const float *heatmap, const int f1, const int f2, const int f3, const float *pafmap) {
//    const int THRE_CNT = 4;
//    const double THRESH_PAF = 0.40;
    vector<Peak> peak_infos[NUM_PART];
	gather_peak_infos(p1, p2, p3, peaks, h2, h3, heatmap, peak_infos);
	gather_peak_infos_line(peak_infos);
...
void gather_peak_infos(const int p1, const int p2, const int p3, const float * peaks, const int h2, const int h3, const float * heatmap, std::vector<Peak> peak_infos[NUM_PART]) {
	int peak_cnt = 0;
	for (int part_id = 0; part_id < NUM_PART; part_id++) {
		for (int y = 0; y < p1; y++) {
			for (int x = 0; x < p2; x++) {
	...
	for (int pair_id = 0; pair_id < COCOPAIRS_SIZE; pair_id++) {
		...
		for (int conn_id = 0; conn_id < (int)conns.size(); conn_id++) {
			...
			if (found == 1) {
				...
				int membership = 0;
				for (int subset_id = 0; subset_id < NUM_PART; subset_id++) {
				...
				if (membership == 0) {
					for (int subset_id = 0; subset_id < NUM_PART; subset_id++) subset[subset_idx1][subset_id] += (subset[subset_idx2][subset_id] + 1);
